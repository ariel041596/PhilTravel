<template>
  <div class="bg-white ">
    ...
    <div class=" q-pa-md">
      <q-form class="text-center" @submit.prevent="editIti">
        <q-input
          :rules="[val => !!val]"
          lazy-rules
        ...
        ></q-input>
        <q-input type="file" class="col q-mb-none q-pb-sm" @change="upload">
        ...
        <div class="q-mt-md">
          <q-btn
            ...
            type="submit"
            no-caps
            :label="!complete ? 'Please fill up details' : 'Edit Itinerary'"
          ></q-btn>
        </div>
      </q-form>
    </div>
  ...
  methods: {
    ...
    editIti() {
      ...
      this.$firestore.itineraries
        .doc(this.itinerary.id)
        .update(this.itinerary)
        .then(response => {
          this.$q.loading.hide();
          this.$emit("close");
        })
